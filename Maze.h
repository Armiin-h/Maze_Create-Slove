...
public:
	Maze(int row,int column):Ground(row+2,column+2),Rat(row*column){
		...
		cout<<"(SHOROE BARNAME RA AZ (1,1) DAR NAZAR BEGIRID!)\n";
		Point temp;
		do{
			Maze::Draw(this);
			cout<<"SATR :";cin>>temp.x;
			cout<<"SOTON :";cin>>temp.y;
			Ground.Set(temp.x,temp.y).Flag()=Denied;
			Rat.Block.Push(Ground.Set(temp.x,temp.y));
			cout<<"EDAME MIDAHID?[Yes(y) or No(n)]:";cin>>decide;
		}while(decide!='n');
		Maze::Draw(this);
	}
	static Maze<>* Create(){
	...
	static bool Menu()
	{
		Maze<>* Game;
		cout<<"\tMaze Game\n";
		cout<<"1)SAKHTE MAZE :\t2)HALLE MAZE :";
		char select;
		cin>>select;
		switch (select)
		{
	...
	static void Solve(Maze<> * Game){
		cout<<"\tSolve"<<endl;
		cout<<"1)HAL BA Stack\t2)HAL BA SAF  :";
		char decide;
		cin>>decide;
		switch(decide){
